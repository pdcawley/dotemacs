...
(defun pdc--resolve-hook-forms (hooks)
  "Converts a list of modes into a list of hook symbols.

If a mode is quoted, it is left as is. If the entire HOOKS list is quoted, the
list is returned as-is."
  (declare (pure t) (side-effect-free t))
  (let ((hook-list (pdc-enlist (pdc-unquote hooks))))
...
(defun pdc--setq-hook-fns (hooks rest &optional singles)
  ...
  (cl-loop with vars = (let ((args rest)
                         ...
                         (while args
                           (push (if singles
                                 ...
                                 vars))
                         (nreverse vars))
           for hook in (pdc--resolve-hook-forms hooks)
           for mode = (string-remove-suffix "-hook" (symbol-name hook))
           append
           (cl-loop for (var . val) in vars
                    collect
                    (list var val hook
                          (intern (format "pdc--setq-%s-for-%s-h"
                                          var mode))))))
...
(defun pdc-enlist (exp)
  "Return EXP wrapped in a list, or as-is if already a list."
  (declare (pure t) (side-effect-free t))
  (if (proper-list-p exp) exp (list exp)))

(defun pdc-keyword-intern (str)
  "Converts STR (a string) into a keyword (`keywordp')."
  (declare (pure t) (side-effect-free t))
  (cl-check-type str string)
  (intern (concat ":" str)))

(defun pdc-keyword-name (keyword)
  "Returns the string name of KEYWORD (`keywordp') minus the leading colon."
  (declare (pure t) (side-effect-free t))
  (cl-check-type keyword keyword)
  (substring (symbol-name keyword) 1))

(defmacro pdc-log (format-string &rest args)
  "Log to *Messages* if `pdc-debug-p' is on.
Does not display text in echo area, but still logs to *Messages*. Accepts the
same arguments as `message'."
  `(when pdc-debug-p
     (let ((inhibit-message (active-minibuffer-window)))
       (message
        ,(concat (propertize "PDC " 'face 'font-lock-comment-face)
                 (when (bound-and-true-p pdc--current-module)
                   (propertize
                    (format "[%s/%s] "
                            (pdc-keyword-name (car pdc--current-module))
                            (cdr pdc--current-module))
                    'face 'warning))
                 format-string)
        ,@args))))

(defalias 'pdc-partial #'apply-partially)

(defun pdc-rpartial (fn &rest args)
...
(defun pdc-lookup-key (keys &rest keymaps)
  "Like `lookup-key', but search active keymaps if KEYMAP is omitted."
  (if keymaps
      (cl-some (pdc-rpartial #'lookup-key keys) keymaps)
    (cl-loop for keymap
...
(defun pdc-load-envvars-file (file &optional noerror)
       ...
       (unless debug-on-error
         (lwarn hook :error "Error running hook %S because: %s"
                (if (symbolp (cadr e))
                  ...
                  (cadr e))
                (caddr e)))
       (signal 'pdc-hook-error (cons hook (cdr e)))))))

(defun pdc-run-hook-on (hook-var trigger-hooks)
...
(defmacro wk-cmd! (wk-desc &rest body)
  "Returns (list (cmd! ,@BODY) :which-key ,WK-DESC
A factory for quickly producing interactions commands with which-key names"
  (declare (doc-string 1) (pure t) (side-effect-free t))
  `(list (cmd! ,@body) :which-key ,wk-desc))
...
(defmacro wk-cmd!! (desc cmd &optional prefix-arg &rest args)
  "Returns an cmd!! and its which-key description suitable for use in general-def"
  (declare (doc-string 1) (pure t) (side-effect-free t))
  `(list (cmd!! ,cmd ,prefix-arg ,@args)
	 :which-key ,desc))
...
(defmacro pred! (&rest body)
  "Expands to `(lambda (it) ,@body) - for making predicate functions"
  (declare (doc-string 1) (indent 2))
  `(lambda (it)
     ,@body))
...
4. Prevents eager expansion pulling in autoloaded macros all at once"
  ...
  (if (symbolp package)
      (unless (memq package (bound-and-true-p pdc-disabled-packages))
...
(defun pdc--handle-load-error (e target path)
  (let* ((source (file-name-sans-extension target))
         (err (cond ((not (featurep 'core))
                     (cons 'error (file-name-directory path)))
                    ((file-in-directory-p source pdc-core-dir)
                     (cons 'pdc-error pdc-core-dir))
                    ((file-in-directory-p source pdc-private-dir)
                     (cons 'pdc-private-error pdc-private-dir))
                    ((cons 'pdc-module-error pdc-emacs-dir)))))
    (signal (car err)
...
If NOERROR is non-nil, don't throw an error if the file doesn't exist."
  (let* ((path (or path
    ...
    `(condition-case-unless-debug e
       ...
       (pdc-error (signal (car e) (cdr e)))
       (error (pdc--handle-load-error e ,file ,path)))))
...
serve as a predicated alternative to `after!'."
  ...
  `(if ,condition
       (progn ,@body)
     ,(let ((fn (intern (format "pdc--delay-form-%s-h" (sxhash (cons condition body))))))
        `(progn
...
reverse this and trigger `after!' blocks at a more reasonable time."
  (let ((advice-fn (intern (format "pdc--defer-feature-%s-a" feature))))
    `(progn
...
advised)."
  ...
  (let ((append (if (eq (car forms) :after) (pop forms)))
        ...
        (fn (intern (format "pdc--transient-%d-h"
                            (put 'add-transient-hook! 'counter
                                 (1+ (or (get 'add-transient-hook! 'counter)
                                         0)))))))
    `(let ((sym ,hook-or-function))
       (defun ,fn (&rest _)
         ,(format "Transient hook for %S" (pdc-unquote hook-or-function))
         ,@forms
         (let ((sym ,hook-or-function))
...
\(fn HOOKS [:append :local [:depth N]] FUNCTIONS-OR-FORMS...)"
  (declare (indent (lambda (indent-point state)
           ...
           (debug t))
  (let* ((hook-forms (pdc--resolve-hook-forms hooks))
         (func-forms ())
         (defn-forms ())
...
\(fn HOOKS &rest [SYM VAL]...)"
  (declare (indent 1))
  (macroexp-progn
   (cl-loop for (var val hook fn) in (pdc--setq-hook-fns hooks var-vals)
...
\(fn HOOKS &rest [SYM VAL]...)"
  ...
  (macroexp-progn
   (cl-loop for (_var _val hook fn)
            in (pdc--setq-hook-fns hooks vars 'singles)
...
\(fn SYMBOL ARGLIST &optional DOCSTRING &rest [WHERE PLACES...] BODY\)"
  ...
  (let (where-alist)
    (while (keywordp (car body))
      (push `(cons ,(pop body) (pdc-enlist ,(pop body)))
            where-alist))
    `(progn
...
(defun pdc-call-process (command &rest args)
  "Execute COMMAND with ARGS synchronously.

Return (STATUS . OUTPUT) when it is done, where STATUS returned error
code of the process and STDOUT is its stdout output."
  (with-temp-buffer
