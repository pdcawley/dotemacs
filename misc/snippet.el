...
(defcustom snippet-cleanup-hook nil
  "Functions to run after finishing with a snippet."
  :type 'hook
  :group 'snippet)
...
(define-key snippet-map [tab]                   'snippet-next-field)
(define-key snippet-map [(control ?i)]          'snippet-next-field)
...
This effectively reverts the snippet to normal text in the buffer."
  (when snippet
    ...
    (setq snippet nil)
    (run-hooks 'snippet-cleanup-hook)))
