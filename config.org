...
(use-package org-roam
  ...
  (org-roam-capture-templates
   `(("d" "default" plain
     ...
     ("p" "project" plain
      ...
      :unnarrowed f)
     ("s" "song" plain
      (file ,(expand-file-name "Templates/SongTemplate.org" org-roam-directory))
      :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n#+filetags: Song\n")
      :unnarrowed f)
     )
   )
  ...
  (defun pdc/org-roam-filter-by-tag (tag-name)
    (lambda (node)
      (member (tag-name (org-roam-node-tags node)))))

  (defun pdc/org-roam-list-notes-by-tag (tag-name)
    (mapcar #'org-roam-node-file
            (seq-filter
             (pdc/org-roam-filter-by-tag tag-name)
             (org-roam-node-list))))

  (defun pdc/org-roam-refresh-agenda-list ()
    (interactive)
    (setq org-agenda-files (pdc/org-roam-list-notes-by-tag "Project")))

  (defun pdc/org-roam-project-finalize-hook ()
    "Adds the cpatured template to `org-agenda-files' if the capture was not aborted."
    (remove-hook 'org-capture-after-finalize-hook #'pdc/org-roam-project-finalize-hook)

    (unless org-note-abort
      (with-current-buffer (org-capture-get :buffer)
        (add-to-list 'org-agenda-files (buffer-file-name)))))

  (defun pdc/org-roam-find-project ()
    (interactive)
    (add-hook 'org-capture-after-finalize-hook #'pdc/org-roam-project-finalize-hook)
    )
  )
