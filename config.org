...
#+BEGIN_SRC emacs-lisp
  ...
  (defun bindings|expand-define-prefix (desc key docstr)
    (let* ((descname (symbol-name desc))
           ...
           (prefix-variable-name (intern (format "%s-prefix-key" descname)))
      ...
      `(progn
         ...
         (defvar ,prefix-variable-name ,key
           ,(format "prefix for %s - use with `pdc|with-leader' type commands" doc))
         (defvar ,mode-leader-variable-name (format "%s %s" mode-leader-key ,key)
           ,(format "mode-leader-key + prefix for %s" doc))
         (general-define-key
          ,leader-variable-name '(:ignore t :which-key ,(symbol-name desc))))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package general
    ...
    (bindings|define-prefix cursor "c" "cursor/mark")
    (bindings|define-prefix elisp "e")
    (bindings|define-prefix files "f")
    (bindings|define-prefix emacs-d "f e" "emacs")
    (bindings|define-prefix jump "j" "jumping around")
    (bindings|define-prefix org "o")
    (bindings|define-prefix package "P")
    (bindings|define-prefix replace "r" "replace")
    (bindings|define-prefix rectangle "R")
    (bindings|define-prefix search "s" "searching")
    (bindings|define-prefix toggle "T" "toggles")  
    (bindings|define-prefix transpose "t" "transpose")
...
#+BEGIN_SRC emacs-lisp
  (req-package multiple-cursors
    ...
    :config
    (pdc|general-bind-hydra mc cursor-prefix-key
      ...
      ("s" mc/sort-regions))
    (pdc|with-leader :infix cursor-prefix-key
...
#+BEGIN_SRC emacs-lisp
  ...
  (pdc|with-leader :prefix emacs-d-leader-key
...
#+BEGIN_SRC emacs-lisp
    (req-package swiper
      ...
      :general
      (pdc|with-leader :infix search-prefix-key
       "s" 'swiper
       "S" 'swiper-all)
      :config
...
#+BEGIN_SRC emacs-lisp
  ...
  (req-package counsel
    ...
    (pdc|with-leader
     ...
     "e L" '(counsel-load-library :which-key "load library")
...
#+BEGIN_SRC emacs-lisp
  ...
  (req-package pdc-helpers
    ...
    (pdc|with-leader
     ...
     "e i"    '(pdc|visit-ielm :which-key "ielm")
     "e R"    '(pdc|eval-and-replace :which-key "eval and replace"))
    :init
